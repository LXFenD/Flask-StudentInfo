...
class AliyunVodUploader:
    ...
    def __createOssClient(self, accessKeyId, accessKeySecret, securityToken, endpoint, bucket):
        ...
        self.__bucketClient = oss2.Bucket(auth, endpoint, bucket, connect_timeout=self.__connTimeout, enable_crc=self.__EnableCrc)
    ...
    def __resumableUpload(self, srcFilePath, destObject, videoId, headers):
        ...
        return oss2.resumable_upload(self.__bucketClient, destObject, srcFilePath,
        ...
        if fileSize >= self.__multipartThreshold:
            uploader = _VodResumableUploader(self.__bucketClient, destObject, srcFilePath, videoId, fileSize,
                                      self.__multipartPartSize, self.__multipartThreadsNum, headers,
                                             self.uploadProgressCallback, self.__refreshUploadAuth)
            result = uploader.upload()
        else:
            with open(AliyunVodUtils.toUnicode(srcFilePath), 'rb') as f:
    ...
    def __refreshUploadAuth(self, videoId):
        ...
        uploadAuth = uploadInfo['UploadAuth']
        uploadAddress = uploadInfo['UploadAddress']
        return self.__createOssClient(uploadAuth['AccessKeyId'], uploadAuth['AccessKeySecret'], uploadAuth['SecurityToken'],
                               uploadAddress['Endpoint'], uploadAddress['Bucket'])
...
class _VodResumableUploader:
    def __init__(self, bucket, key, fileName, videoId, totalSize,
                 partSize, threadsNum, headers, progressCallback, refreshAuthCallback):
